...
class VFAGripper(DynamixelHelloXL430):
    """
    API to the VFA Gripper
    The gripper motion is non-linear w.r.t to motor motion due to its design
    ...
    def __init__(self, chain=None):
        DynamixelHelloXL430.__init__(self, 'vfa_gripper', chain)
    ...
    def open(self):
        self.enable_pos()
        x_r = self.ticks_to_world_rad(self.params['range_t'][0])
        DynamixelHelloXL430.move_to(self,x_des=x_r)

    def close(self):
	self.enable_pwm()
        DynamixelHelloXL430.set_pwm(self, self.params['pwm_homing'][1])
        ts=time.time()
        time.sleep(1.0)
        timeout=False
